--[[
API:
args -> размер X и размер Y поля, которое будет обрабатываться
...
метод field.farm() -> создает бесконечный цикл уборки поля через определенный промежуток времени. использует методы field.harvest() и field.cleanInventory()
метод field.harvest() -> создает конечный цикл, который обходит поле определенного размера
]]--
...
--структура скрипта:
	--уборка урожая с поля
	--перемещение с одного поля на другое
	--складирование инвентаря

	--здесь будет записан весь скрипт так, что его необходимо будет только активировать
...
--интерфейс взаимодействия с внешними программами
